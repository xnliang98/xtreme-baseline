...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id=None):
    ...
    if args.local_rank in [-1, 0]:
        tb_writer.close()
    return lobal_step, tr_loss / global_step

def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", lang="en", lang2id=None, print_result=True):
    eval_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode=mode, lang=lang, lang2id=lang2id)[:20]
...
args = dotdict(yaml.safe_load(open("run_tag.yml")))
...
config = XLMRobertaConfig.from_pretrained(args.config_name if args.config_name else args.model_name_or_path,
                      num_labels=num_labels,
                      cache_dir=args.cache_dir if args.cache_dir else None)
tokenizer = XLMRobertaTokenizer.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
                        do_lower_case=args.do_lower_case,
                        cache_dir=args.cache_dir if args.cache_dir else None)

if args.init_checkpoint:
    logger.info("loading from init_checkpoint={}".format(args.init_checkpoint))
    model = XLMRobertaForTokenClassification.from_pretrained(args.init_checkpoint,
                                        config=config,
                                        cache_dir=args.init_checkpoint)
else:
    logger.info("loading from cached model = {}".format(args.model_name_or_path))
    model = XLMRobertaForTokenClassification.from_pretrained(args.model_name_or_path,
...
logger.info("Training/evaluation parameters %s", args)# Training
if args.do_train:
...
if args.do_eval and args.local_rank in [-1, 0]:
    # tokenizer = XLMRobertaTokenizer.from_pretrained(args.output_dir + "checkpoint-10", do_lower_case=args.do_lower_case)
...
if args.do_predict and args.local_rank in [-1, 0]:
    model = XLMRobertaForTokenClassification.from_pretrained(best_checkpoint)
    model.to(args.device)
